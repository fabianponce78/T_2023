...
--Select   * from [JIRA].[dbo].[V_JIRA_STORIES] order by DT

SELECT        
	DT, DT_SPRINT, SPRINT, ParentStory, Issue_Type, Summary, [Key], 
	Assignee, Reporter, Priority, Status, 
	Status2, Resolution, 
	Created, Updated, Due_date, parent, 
	Resolved, Status_Category_Changed, 
	[Time Resolution] AS Linked_Issues, 
	Wk_Created AS Parent_Link, 
	Wk_Resolved AS [Time Resolution], 
	Wk_Created_Resolved AS Wk_Created
...
--						 ELSE '' END AS Status2, ISNULL(J.Resolution, '') AS Resolution, ISNULL(CONVERT(varchar, J.Created, 23), '') AS Created, 
--						 ISNULL(CONVERT(varchar, J.Updated, 23), '')                            AS Updated, 
--						 ISNULL(J.Due_date, '') AS Due_date, ISNULL(J.parent, '') AS parent, 
--						 ISNULL(CONVERT(varchar, J.Resolved, 23), '') AS Resolved, ISNULL(CONVERT(varchar, J.Status_Category_Changed, 23), '') 
--                         AS Status_Category_Changed, ISNULL(J.Linked_Issues, '') AS Linked_Issues, 
--						 ISNULL(J.Parent_Link, '') AS Parent_Link, CASE WHEN [Status] = 'Done' THEN DATEDIFF(d, created, Resolved) ELSE '' END AS [Time Resolution], 
